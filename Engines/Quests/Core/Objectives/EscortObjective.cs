...
namespace Server.Engines.Quests.Objectives
{
	public class EscortObjective : BaseObjective
	{
		...
		public override bool CanOffer(IQuestGivers quester, PlayerMobile pm, bool message)
		{
			...
			if (context != null)
			{
				foreach (QuestInstance instance in context.QuestInstances)
				{
					if (instance.Quest.IsEscort)
					{
						if (message)
							QuestSystem.Tell(quester, pm, 500896); // I see you already have an escort.
			...
			if (nextEscort > DateTime.UtcNow)
			{
				if (message)
				{
					...
					if (minutes == 1)
						QuestSystem.Tell(quester, pm, "You must rest 1 minute before we set out on this journey.");
					else
						QuestSystem.Tell(quester, pm, 1071195, minutes.ToString()); // You must rest ~1_minsleft~ minutes before we set out on this journey.
				}
		...
		public override BaseObjectiveInstance CreateInstance(QuestInstance instance)
		{
	...
	public class EscortObjectiveInstance : BaseObjectiveInstance
	{
		...
		public EscortObjectiveInstance(EscortObjective objective, QuestInstance instance)
			: base(instance, objective)
		{
			...
			if (QuestSystem.Debug && m_Escort == null && instance.Quester != null)
		...
		private void CheckDestination()
		{
			...
			QuestInstance instance = Instance;
			...
			else if (m_Objective.Destination.Contains(m_Escort))
			{
				...
				// Auto claim reward
				QuestSystem.OnDoubleClick(m_Escort, pm);
			}
