...
namespace Server.Engines.Quests.Objectives
{
	public class CollectObjective : BaseObjective
	{
		...
		public CollectObjective(int amount, Type type, TextDefinition name)
		{
			...
			if (QuestSystem.Debug && ShowDetailed && name.Number > 0)
			{
	...
	public class CollectObjectiveInstance : BaseObjectiveInstance
	{
		...
		public CollectObjectiveInstance(CollectObjective objective, QuestInstance instance)
		...
		public override void OnQuestCancelled()
		{
			...
			foreach (Item item in items)
			{
				if (item.QuestItem && !QuestSystem.CanMarkQuestItem(pm, item, checkType)) // does another quest still need this item? (OSI just unmarks everything)
