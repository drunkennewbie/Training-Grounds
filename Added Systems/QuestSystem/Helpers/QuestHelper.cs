	...
	public class QuestHelper
	{
		...
		public static void ToggleQuestItems(Mobile from, object obj)
		{
			if (from is PlayerMobile)
			{
				...
				player.BeginTarget(-1, false, TargetFlags.None, new TargetCallback(ToggleQuestItems));
			}
		}
			...
			for (int i = 0; i < 10; i++)
			{
				...
				if (quest != null)
				{
					...
					else if (quester is Mobile && message)
					{
						...
						{
							((NewQuester)quester).OnOfferFailed();
						}
		...
		public static bool CheckDoneOnce(PlayerMobile player, Type questType, Mobile quester, bool message)
		{
			if (player.DoneQuests.Any(x => x.QuestType == questType))
			{
				...
				{
					quester.SayTo(player, 1075454); // I can not offer you the quest again.
				}
			...
			for (int i = 0; i < quests.Length; i++)
			{
				for (int j = 0; j < player.Quests.Count; j++)
				{
					...
					if (quests[i].IsAssignableFrom(quest.GetType()))
					{
						if (quest.Completed)
						{
							player.SendGump(new NewQuestGump(quest, NewQuestGump.Section.Complete, false, true));
						}
						else
						{
							player.SendGump(new NewQuestGump(quest, NewQuestGump.Section.InProgress, false));
							quest.InProgress();
						}
					...
					if (objective is DeliverObjective)
					{
						...
						if (deliver.Update(vendor))
						{
							if (quest.Completed)
							{
								...
								if (vendor is NewQuester)
									player.SendGump(new NewQuestGump(player, quest, NewQuestGump.Section.Complete, false, true, (NewQuester)vendor));
								else
									player.SendGump(new NewQuestGump(quest, NewQuestGump.Section.Complete, false, true));
							}
		...
		public static void CompleteQuest(PlayerMobile from, BaseQuest quest)
		{
			...
			from.SendGump(new NewQuestGump(quest, NewQuestGump.Section.Complete, false, true));
		...
		{
			Console.WriteLine(String.Format("Checking Slayer Object"));
